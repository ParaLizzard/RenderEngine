...
namespace Engine {
    ...
    void IBL::generateIrradiance()
    {
        ...
        allocInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
        vmaCreateImage(
            device.getAllocator(), &imageCI, &allocInfo, &irradianceCube.image, &irradianceAllocation, nullptr);
        ...
        for (uint32_t m = 0; m < numMips; m++) {
            for (uint32_t f = 0; f < 6; f++) {
                ...
                vkCmdBindDescriptorSets(
                    cmdBuf, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelinelayout, 0, 1, &descriptorset, 0, nullptr);

                megaBuffer.bind(cmdBuf);
                vkCmdDrawIndexed(
                    cmdBuf, cube.subMesh.indexCount, 1, cube.subMesh.firstIndex, cube.subMesh.vertexOffset, 0);
    ...
    void IBL::generatePrefilter()
    {
        ...
        allocInfo.usage = VMA_MEMORY_USAGE_GPU_ONLY;
        vmaCreateImage(
            device.getAllocator(), &imageCI, &allocInfo, &prefilteredCube.image, &prefilterAllocation, nullptr);
