...
namespace Engine
{
        ...
        renderGraph(renderGraph)
    {
        ...
        for (size_t i = 0; i < Renderer::MAX_FRAMES_IN_FLIGHT; i++)
        {
            ...
            packedNormalBuffers[i] = std::make_unique<Buffer>(
                device, sizeof(uint32_t), initialPixelCount, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0
            );

            packedRadianceBuffers[i] = std::make_unique<Buffer>(
                device, sizeof(uint32_t), initialPixelCount, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0
            );
        }
    ...
    void MaterialPassNode::setup(RenderGraphBuilder& renderGraph)
    {
        ...
        renderGraph.readImage("SsaoBlurImage", VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                              VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_READ_BIT);

        renderGraph.writeBuffer("PackedNormals", VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_WRITE_BIT);
        renderGraph.writeBuffer("PackedRadiances", VK_PIPELINE_STAGE_2_COMPUTE_SHADER_BIT, VK_ACCESS_2_SHADER_WRITE_BIT);
    ...
    void MaterialPassNode::execute(VkCommandBuffer& cmd, FrameInfo& frameInfo)
    {
        ...
        {
            meshBuffers[currentFrame]->writeToBuffer(cachedMeshInfos.data(), cachedMeshInfos.size() * sizeof(GPUMeshInfo), 0);
    ...
    void MaterialPassNode::resolve(RenderGraph& graph, const FrameInfo& frameInfo)
    {
        ...
        if (extent.width != lastWidth || extent.height != lastHeight)
        {
            ...
            for (size_t i = 0; i < Renderer::MAX_FRAMES_IN_FLIGHT; i++)
            {
                worldPositionBuffers[i] = std::make_unique<Buffer>(device, sizeof(WorldData), pixelCount, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0);
                //compactMaterialBuffers[i] = std::make_unique<Buffer>(device, sizeof(CompactMaterial), pixelCount, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0);
                packedNormalBuffers[i] = std::make_unique<Buffer>(device, sizeof(uint32_t), pixelCount, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0);
                packedRadianceBuffers[i] = std::make_unique<Buffer>(device, sizeof(uint32_t), pixelCount, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0);
            }
        }
