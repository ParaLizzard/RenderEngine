...
namespace Engine
{
    VisibilityPassNode::VisibilityPassNode(Device& device, Renderer& renderer, Model& megaBuffer, CullPassNode& cullPass) :
    device(device),
    ...
    void VisibilityPassNode::setup(RenderGraphBuilder& renderGraph)
    {
        ...
        renderGraph.createTransientImage("VisBuffer", VK_FORMAT_R32G32_UINT, currentExtent, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);

        renderGraph.readBuffer("CullCompactedIndirectCommands",
                               VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT,
                               VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT);

        renderGraph.readBuffer("CullDrawCount",
                               VK_PIPELINE_STAGE_2_DRAW_INDIRECT_BIT,
                               VK_ACCESS_2_INDIRECT_COMMAND_READ_BIT);
        ...
        renderGraph.writeImage("DepthImage",
    ...
    void VisibilityPassNode::execute(VkCommandBuffer& cmd, FrameInfo& frameInfo)
{
    ...
    void VisibilityPassNode::createPipeline()
    {
        ...
        vkCreateGraphicsPipelines(device.getDevice(), device.getPipelineCache(), 1 , &pipelineInfo, VK_NULL_HANDLE, &pipeline);
