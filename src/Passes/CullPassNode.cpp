...
namespace Engine
{
    CullPassNode::CullPassNode(Device& device, Renderer& renderer, Model& megaBuffer) : device(device),
        megaBuffer(megaBuffer), renderer(renderer)
    {
        ...
        {
            cpuObjectSSBOs[i] = std::make_unique<Buffer>(device, sizeof(ObjectData), MAX_SCENE_OBJECTS,
                                                         VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU,
                                                         VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, 0);
            cpuIndirectCommandBuffers[i] = std::make_unique<Buffer>(device, sizeof(VkDrawIndexedIndirectCommand),
                                                                    MAX_SCENE_OBJECTS, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                                                    VMA_MEMORY_USAGE_CPU_TO_GPU,
                                                                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, 0);
            gpuObjectSSBOs[i] = std::make_unique<Buffer>(device, sizeof(ObjectData), MAX_SCENE_OBJECTS,
                                                         VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                                                         VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_ONLY,
                                                         VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, 0);
            gpuIndirectCommandBuffers[i] = std::make_unique<Buffer>(device, sizeof(VkDrawIndexedIndirectCommand),
                                                                    MAX_SCENE_OBJECTS,
                                                                    VK_BUFFER_USAGE_TRANSFER_DST_BIT |
            ...
            gpuCompactedIndirectCommandBuffers[i] = std::make_unique<Buffer>(device, sizeof(VkDrawIndexedIndirectCommand),
                                                                             MAX_SCENE_OBJECTS,
                                                                             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
            ...
            gpuDrawCountBuffers[i] = std::make_unique<Buffer>(device, sizeof(uint32_t), 1,
                                                              VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                                                              VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                                                              VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
                                                              VMA_MEMORY_USAGE_GPU_ONLY,
    ...
    void CullPassNode::execute(VkCommandBuffer& cmd, FrameInfo& frameInfo)
    {
        ...
        if (framesToUpdate > 0)
        {
            ...
            {
                cpuObjectSSBOs[currentFrame]->writeToBuffer(objectDataArray.data(),
                                                            objectDataArray.size() * sizeof(ObjectData), 0);
                cpuObjectSSBOs[currentFrame]->flush(VK_WHOLE_SIZE, 0);
                cpuIndirectCommandBuffers[currentFrame]->writeToBuffer(indirectCommandsArray.data(),
                                                                       indirectCommandsArray.size() * sizeof(VkDrawIndexedIndirectCommand), 0);
